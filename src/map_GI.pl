:- dynamic(player_X/1).
:- dynamic(player_Y/1).
:- dynamic(stamina/1).

slime_Z(5,23).
slime_Z(5,24).
slime_Z(5,25).
slime_Z(6,23).
slime_Z(6,24).
slime_Z(6,25).
slime_Z(7,22).
slime_Z(7,23).
slime_Z(7,24).
slime_Z(7,25).
slime_Z(7,26).
slime_Z(8,23).
slime_Z(8,24).
slime_Z(8,25).
slime_Z(9,23).
slime_Z(9,24).
slime_Z(10,23).
slime_Z(10,24).
slime_Z(10,25).
slime_Z(11,21).
slime_Z(11,22).
slime_Z(11,23).
slime_Z(11,24).
slime_Z(12,22).
slime_Z(12,23).
slime_Z(13,23).
slime_Z(13,24).
slime_Z(14,24).
slime_Z(15,24).
slime_Z(15,25).
slime_Z(15,26).
slime_Z(15,27).
slime_Z(16,24).
slime_Z(16,25).
slime_Z(19,13).
slime_Z(20,9).
slime_Z(20,10).
slime_Z(20,12).
slime_Z(20,13).
slime_Z(20,14).
slime_Z(21,9).
slime_Z(21,10).
slime_Z(21,11).
slime_Z(21,12).
slime_Z(21,13).
slime_Z(21,14).
slime_Z(22,8).
slime_Z(22,9).
slime_Z(22,12).
slime_Z(22,13).
slime_Z(22,14).
slime_Z(23,7).
slime_Z(23,8).
slime_Z(23,9).
slime_Z(23,10).
slime_Z(24,8).
slime_Z(24,9).

goblin_Z(2,21).
goblin_Z(2,22).
goblin_Z(3,21).
goblin_Z(3,22).
goblin_Z(4,19).
goblin_Z(4,20).
goblin_Z(4,21).
goblin_Z(8,6).
goblin_Z(9,5).
goblin_Z(9,6).
goblin_Z(10,4).
goblin_Z(10,5).
goblin_Z(10,6).
goblin_Z(10,11).
goblin_Z(11,5).
goblin_Z(11,6).
goblin_Z(11,7).
goblin_Z(11,10).
goblin_Z(11,11).
goblin_Z(11,12).
goblin_Z(12,4).
goblin_Z(12,5).
goblin_Z(12,11).
goblin_Z(13,3).
goblin_Z(13,4).
goblin_Z(13,10).
goblin_Z(13,11).
goblin_Z(13,12).
goblin_Z(13,13).
goblin_Z(14,1).
goblin_Z(14,2).
goblin_Z(14,3).
goblin_Z(14,11).
goblin_Z(14,12).
goblin_Z(15,1).
goblin_Z(15,2).
goblin_Z(15,3).
goblin_Z(16,1).
goblin_Z(17,16).
goblin_Z(18,16).
goblin_Z(18,17).
goblin_Z(18,18).
goblin_Z(19,17).
goblin_Z(19,18).

wolf_Z(1,1).
wolf_Z(1,2).
wolf_Z(1,3).
wolf_Z(1,4).
wolf_Z(1,5).
wolf_Z(1,6).
wolf_Z(1,7).
wolf_Z(1,8).
wolf_Z(1,9).
wolf_Z(1,10).
wolf_Z(1,11).
wolf_Z(1,12).
wolf_Z(1,13).
wolf_Z(1,14).
wolf_Z(2,1).
wolf_Z(2,2).
wolf_Z(2,3).
wolf_Z(2,4).
wolf_Z(2,5).
wolf_Z(2,6).
wolf_Z(2,7).
wolf_Z(2,8).
wolf_Z(2,9).
wolf_Z(2,10).
wolf_Z(2,11).
wolf_Z(2,12).
wolf_Z(3,1).
wolf_Z(3,2).
wolf_Z(3,3).
wolf_Z(3,4).
wolf_Z(3,5).
wolf_Z(3,6).
wolf_Z(3,7).
wolf_Z(3,8).
wolf_Z(3,9).
wolf_Z(3,10).
wolf_Z(4,1).
wolf_Z(4,2).
wolf_Z(4,3).
wolf_Z(4,4).
wolf_Z(4,5).
wolf_Z(4,6).
wolf_Z(4,7).
wolf_Z(4,8).
wolf_Z(5,1).
wolf_Z(5,2).
wolf_Z(5,3).
wolf_Z(5,4).
wolf_Z(6,1).
wolf_Z(6,2).
wolf_Z(6,3).
wolf_Z(7,1).
wolf_Z(7,2).
wolf_Z(7,3).
wolf_Z(8,1).
wolf_Z(8,2).
wolf_Z(9,2).
wolf_Z(8,17).
wolf_Z(9,16).
wolf_Z(9,17).
wolf_Z(10,16).
wolf_Z(10,17).
wolf_Z(10,18).
wolf_Z(11,17).
wolf_Z(11,18).
wolf_Z(26,1).
wolf_Z(27,1).
wolf_Z(28,1).
wolf_Z(28,2).
wolf_Z(29,1).
wolf_Z(29,2).
wolf_Z(29,3).
wolf_Z(30,1).
wolf_Z(30,2).
wolf_Z(30,3).
wolf_Z(31,1).
wolf_Z(31,2).
wolf_Z(31,3).
wolf_Z(31,4).
wolf_Z(31,11).
wolf_Z(32,1).
wolf_Z(32,2).
wolf_Z(32,3).
wolf_Z(32,3).
wolf_Z(32,9).
wolf_Z(32,10).
wolf_Z(32,11).
wolf_Z(33,1).
wolf_Z(33,2).
wolf_Z(33,3).
wolf_Z(33,4).
wolf_Z(33,7).
wolf_Z(33,8).
wolf_Z(33,9).
wolf_Z(33,10).
wolf_Z(33,11).
wolf_Z(33,12).

quest(9,20).
quest(28,14).

shop(8,20).
shop(27,13).

monstadt(8,19).

liyue_harbor(28,13).

qingyun_peak(19,5).

boss_dungeon(6,14).

water(1,16).
water(1,17).
water(1,19).
water(1,20).
water(1,23).
water(1,24).
water(1,25).
water(1,26).
water(1,27).
water(1,28).
water(1,29).
water(1,30).
water(1,31).
water(1,32).
water(1,33).
water(2,17).
water(2,18).
water(2,19).
water(2,20).
water(2,24).
water(2,25).
water(2,26).
water(2,27).
water(2,28).
water(2,29).
water(2,30).
water(2,31).
water(2,32).
water(2,33).
water(3,17).
water(3,18).
water(3,23).
water(3,24).
water(3,25).
water(3,26).
water(3,27).
water(3,28).
water(3,29).
water(3,30).
water(3,31).
water(3,32).
water(3,33).
water(4,17).
water(4,23).
water(4,24).
water(4,25).
water(4,26).
water(4,27).
water(4,28).
water(4,29).
water(4,30).
water(4,31).
water(4,32).
water(4,33).
water(5,10).
water(5,18).
water(5,18).
water(5,28).
water(5,29).
water(5,30).
water(5,31).
water(5,32).
water(5,33).
water(6,7).
water(6,19).
water(6,20).
water(6,21).
water(6,27).
water(6,28).
water(6,29).
water(6,30).
water(6,31).
water(6,32).
water(6,33).
water(7,19).
water(7,20).
water(7,21).
water(7,27).
water(7,28).
water(7,29).
water(7,30).
water(7,31).
water(7,32).
water(7,33).
water(8,8).
water(8,10).
water(8,18).
water(8,28).
water(8,29).
water(8,30).
water(8,31).
water(8,32).
water(8,33).
water(9,8).
water(9,9).
water(9,19).
water(9,27).
water(9,28).
water(9,29).
water(9,30).
water(9,31).
water(9,32).
water(9,33).
water(10,1).
water(10,9).
water(10,14).
water(10,20).
water(10,27).
water(10,28).
water(10,29).
water(10,30).
water(10,31).
water(10,32).
water(10,33).
water(11,1).
water(11,2).
water(11,3).
water(11,14).
water(11,26).
water(11,27).
water(11,28).
water(11,29).
water(11,30).
water(11,31).
water(11,32).
water(11,33).
water(12,1).
water(12,3).
water(12,13).
water(12,16).
water(12,25).
water(12,26).
water(12,27).
water(12,28).
water(12,29).
water(12,30).
water(12,31).
water(12,32).
water(12,33).
water(13,8).
water(13,9).
water(13,17).
water(13,26).
water(13,27).
water(13,28).
water(13,29).
water(13,30).
water(13,31).
water(13,33).
water(14,4).
water(14,5).
water(14,6).
water(14,9).
water(14,29).
water(14,30).
water(14,31).
water(14,32).
water(14,33).
water(15,4).
water(15,5).
water(15,9).
water(15,10).
water(15,14).
water(15,15).
water(15,16).
water(15,29).
water(15,30).
water(15,31).
water(15,32).
water(15,33).
water(16,10).
water(16,11).
water(16,14).
water(16,15).
water(16,28).
water(16,29).
water(16,30).
water(16,31).
water(16,32).
water(16,33).
water(17,3).
water(17,11).
water(17,24).
water(17,25).
water(17,26).
water(17,27).
water(17,28).
water(17,29).
water(17,30).
water(17,31).
water(17,32).
water(17,33).
water(18,12).
water(18,14).
water(18,23).
water(18,24).
water(18,25).
water(18,26).
water(18,27).
water(18,28).
water(18,29).
water(18,30).
water(18,31).
water(18,32).
water(18,33).
water(19,7).
water(19,21).
water(19,23).
water(19,24).
water(19,25).
water(19,26).
water(19,27).
water(19,28).
water(19,29).
water(19,30).
water(19,31).
water(19,32).
water(19,33).
water(20,2).
water(20,15).
water(20,19).
water(20,20).
water(20,21).
water(20,22).
water(20,23).
water(20,24).
water(20,25).
water(20,26).
water(20,27).
water(20,28).
water(20,29).
water(20,30).
water(20,31).
water(20,32).
water(20,33).
water(21,1).
water(21,2).
water(21,18).
water(21,19).
water(21,20).
water(21,21).
water(21,22).
water(21,23).
water(21,24).
water(21,25).
water(21,26).
water(21,27).
water(21,28).
water(21,29).
water(21,30).
water(21,31).
water(21,32).
water(21,33).
water(22,2).
water(22,6).
water(22,11).
water(22,16).
water(22,17).
water(22,18).
water(22,19).
water(22,20).
water(22,21).
water(22,22).
water(22,23).
water(22,24).
water(22,25).
water(22,26).
water(22,27).
water(22,28).
water(22,29).
water(22,30).
water(22,31).
water(22,32).
water(22,33).
water(23,6).
water(23,12).
water(23,16).
water(23,17).
water(23,18).
water(23,19).
water(23,22).
water(23,23).
water(23,24).
water(23,25).
water(23,26).
water(23,27).
water(23,28).
water(23,29).
water(23,30).
water(23,31).
water(23,32).
water(23,33).
water(24,2).
water(24,17).
water(24,18).
water(24,20).
water(24,22).
water(24,23).
water(24,24).
water(24,25).
water(24,26).
water(24,27).
water(24,28).
water(24,29).
water(24,30).
water(24,31).
water(24,32).
water(24,33).
water(25,9).
water(25,16).
water(25,17).
water(25,19).
water(25,20).
water(25,23).
water(25,24).
water(25,25).
water(25,26).
water(25,27).
water(25,28).
water(25,29).
water(25,30).
water(25,31).
water(25,32).
water(25,33).
water(26,14).
water(26,15).
water(26,16).
water(26,17).
water(26,18).
water(26,22).
water(26,23).
water(26,24).
water(26,25).
water(26,26).
water(26,27).
water(26,28).
water(26,29).
water(26,30).
water(26,31).
water(26,32).
water(26,33).
water(27,7).
water(27,8).
water(27,14).
water(27,15).
water(27,16).
water(27,17).
water(27,18).
water(27,19).
water(27,20).
water(27,21).
water(27,22).
water(27,23).
water(27,24).
water(27,25).
water(27,26).
water(27,27).
water(27,28).
water(27,29).
water(27,30).
water(27,31).
water(27,32).
water(27,33).
water(28,15).
water(28,16).
water(28,17).
water(28,18).
water(28,19).
water(28,20).
water(28,21).
water(28,22).
water(28,23).
water(28,24).
water(28,25).
water(28,26).
water(28,27).
water(28,28).
water(28,29).
water(28,30).
water(28,31).
water(28,32).
water(28,33).
water(29,16).
water(29,17).
water(29,18).
water(29,19).
water(29,20).
water(29,21).
water(29,22).
water(29,23).
water(29,24).
water(29,25).
water(29,26).
water(29,27).
water(29,28).
water(29,29).
water(29,30).
water(29,31).
water(29,32).
water(29,33).
water(30,15).
water(30,16).
water(30,17).
water(30,18).
water(30,19).
water(30,20).
water(30,21).
water(30,22).
water(30,23).
water(30,24).
water(30,25).
water(30,26).
water(30,27).
water(30,28).
water(30,29).
water(30,30).
water(30,31).
water(30,32).
water(30,33).
water(31,13).
water(31,14).
water(31,15).
water(31,16).
water(31,17).
water(31,18).
water(31,19).
water(31,20).
water(31,21).
water(31,22).
water(31,23).
water(31,24).
water(31,25).
water(31,26).
water(31,27).
water(31,28).
water(31,29).
water(31,30).
water(31,31).
water(31,32).
water(31,33).
water(32,14).
water(32,15).
water(32,16).
water(32,17).
water(32,18).
water(32,19).
water(32,20).
water(32,21).
water(32,22).
water(32,23).
water(32,24).
water(32,25).
water(32,26).
water(32,27).
water(32,28).
water(32,29).
water(32,30).
water(32,31).
water(32,32).
water(32,33).
water(33,14).
water(33,15).
water(33,16).
water(33,17).
water(33,18).
water(33,19).
water(33,20).
water(33,21).
water(33,22).
water(33,23).
water(33,24).
water(33,25).
water(33,26).
water(33,27).
water(33,28).
water(33,29).
water(33,30).
water(33,31).
water(33,32).
water(33,33).

mount(13,21).
mount(14,18).
mount(14,19).
mount(14,20).
mount(14,21).
mount(14,22).
mount(15,17).
mount(15,18).
mount(15,19).
mount(15,20).
mount(15,21).
mount(15,22).
mount(15,23).
mount(16,4).
mount(16,18).
mount(16,19).
mount(16,20).
mount(16,21).
mount(16,22).
mount(17,7).
mount(17,19).
mount(17,20).
mount(17,21).
mount(18,6).
mount(18,9).
mount(18,20).
mount(18,21).
mount(19,2).
mount(19,3).
mount(19,6).
mount(20,3).
mount(20,8).
mount(21,6).
mount(23,14).
mount(24,11).
mount(24,12).
mount(25,12).
mount(27,6).
mount(29,6).
mount(25,12).
mount(30,5).
mount(31,7).
mount(32,4).

initial :-
	asserta(player_X(26)),
	asserta(player_Y(7)),
	asserta(stamina(5))
	.

% Print map
% Mencapai kanan bawah
printX(Y,X) :-
	(
	Y =:= 34, X =:= 34 ->
		write('')
	).

% Print simbol pemain
printX(Y,X) :-
	(
	player_X(TempX), X =:= TempX, player_Y(TempY), Y =:= TempY ->
		write('*P')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol shop
printX(Y,X) :-
	(
	shop(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*S')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol boss_dungeon
printX(Y,X) :-
	(
	boss_dungeon(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*D')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol water
printX(Y,X) :-
	(
	water(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('--')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol mount
printX(Y,X) :-
	(
	mount(TempY,TempX), Y =:= TempY, X =:= TempX ->
		put(47),put(92)
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol Quest Board
printX(Y,X) :-
	(
	quest(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*Q')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol Enemy --> Debugger
printX(Y,X) :-
	(
	enemy(_,_,_,_,_,_,_,TempY,TempX), Y =:= TempY, X =:= TempX ->
		put(164),write('E')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol Wolf Zone
printX(Y,X) :-
	(
	wolf_Z(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('{}')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol Slime Zone
printX(Y,X) :-
	(
	slime_Z(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('()')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print simbol Slime Zone
printX(Y,X) :-
	(
	goblin_Z(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('<>')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print Waypoint
printX(Y,X) :-
	(
	monstadt(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*W')
	),
	NextX is (X + 1),
    printX(Y,NextX).

printX(Y,X) :-
	(
	liyue_harbor(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*W')
	),
	NextX is (X + 1),
    printX(Y,NextX).

printX(Y,X) :-
	(
	qingyun_peak(TempY,TempX), Y =:= TempY, X =:= TempX ->
		write('*W')
	),
	NextX is (X + 1),
    printX(Y,NextX).

% Print batas bawah
printX(Y,X) :-
	(
	Y =:= 34 ->
		write('##')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print batas kiri
printX(Y,X) :-
	(
	X =:= 0 ->
		write('#')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print batas kanan
printX(Y,X) :-
	(
	X =:= 34 ->
		 write('#'),nl
	),
    NextX is 0,
    NextY is (Y + 1),
    printX(NextY,NextX).

% Print batas atas
printX(Y,X) :-
	(
	Y =:= 0 ->
		write('##')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print petak isi
printX(Y,X) :-
	(
	\+ X =:= 0, \+ X =:= 34, \+ Y =:= 0, \+ Y =:= 34 ->
    	write('[]')
	),
    NextX is (X + 1),
    printX(Y,NextX).

% Print arti simbol peta
print_legend :-
	nl,
	write('[] -> Ground\t'),
	write('-- -> Water\t'),
	write('/'),put(92),write(' -> Mountain\t'),
	put(164),write('E -> Enemy'),nl,
	write('() -> Swamp [Slimes\' territoy]\t'),
	write('{} -> Hills [Wild Wolves\' territory]\t'),
	write('<> -> Valley [Goblins\' territory]'),nl,
	write('*P -> Player\t'),
	write('*W -> Waypoint\t'),
	write('*S -> Shop\t'),
	write('*Q -> Quest Board\t'),
	write('*D -> Dungeon'),
	nl.